...
\hline 
Experiment & File & Runtime & Power & Baseline & Detector & Mass \\ \hline \hline
T2K & {\tt T2K.glb} & 2~yr $\nu$/6~yr $\bar{\nu}$ & 0.77~MW & 295~km & WC & 22.5~kt \\ 
T2HK & {\tt T2HK.glb} & 4~yr $\nu$/4~yr $\bar{\nu}$ & 4~MW & 295~km & WC & 500~kt \\ 
NO$\nu$A & {\tt NOvA.glb} & 3~yr $\nu$/3~yr $\bar{\nu}$ & 1.12~MW & 812~km & TASD & 25~kt \\
SPL & {\tt SPL.glb} & 2~yr $\nu$/8~yr $\bar{\nu}$ & 4~MW & 130~km & WC & 500~kt \\ 
\hline
\end{tabular}
\end{center}
...
sample stays {\tt chiSpectrumTilt}. Note, that otherwise the free normalization would be switched off and all events from
...
For the disappearance channels only the QE sample is used since statistics is already quite large and a treatment as for the
appearance channels would only slightly modify the results. The quantitative treatment of systematics is similar to \cite{Ishitsuka:2005qi}. The following rules are 
...
\begin{tabular}{|l|ll|c|c|}
\hline \hline
\multicolumn{3}{|l|}{Appearance (--) -- Spectrum}  & $\sigma_\mathrm{norm}$ & $\sigma_\mathrm{cal}$ \\ \hline
...
The \NOVA\ experiment can be simulated with the file {\tt NOvA.glb}. The description of the disappearance 
...
\item {\tt XNC.dat} (neutral current cross sections)
\ei
The \NOVA\ experiment uses a neutrino beam from the Fermilab NuMI beamline. The source power is $10^{21}$~pot~$\mathrm{yr^{-1}}$, corresponding to a (naively computed) target power of
1.12~MW. For the running times, 3~years $\nu$-running and 3~years $\bar{\nu}$-running 
are assumed. The fiducial mass of the Totally Liquid Scintillator Detector (TASD) is taken to be 
...
four migration matrices (for $\nu_e , \bar{\nu}_e ,\nu_{\mu} ,\bar{\nu}_\mu$) that describe energy smearing 
due to Fermi Motion. The following rules are defined within {\tt SPL.glb}: 
\begin{center}
\begin{tabular}{|l|ll|c|c|}
\hline \hline
\multicolumn{3}{|l|}{Disappearance (+)} & $\sigma_\mathrm{norm}$ & $\sigma_\mathrm{cal}$ \\ \hline 
...
\begin{tabular}{lllllll} 
\hline
Experiment & File & Runtime & th. Power & Baseline & Det. & Mass \\ \hline \hline
Small & {\tt Reactor1.glb} & 5~yr $\bar{\nu}$ & 4~GW & 1.7~km & LS & 20~t \\ 
Large & {\tt Reactor.glb} & 8~yr $\bar{\nu}$ & 10~GW & 1.7~km & LS & 100~t \\
Double Chooz & {\tt D-Chooz\_near.glb} & 5~yr $\bar{\nu}$ & 2$\times$4.2~GW & 0.1~km & LS & 10.16~t \\ 
 & {\tt D-Chooz\_far.glb} & 5~yr $\bar{\nu}$ & 2$\times$4.2~GW & 1.05~km & LS & 10.16~t \\ 
\hline 
\end{tabular}
\end{center}
...
$\mathrm{\mathcal{L} = 400 \,t \, GW\, yr}$, such as for a 20~t detector, a reactor with a thermal power of 4~GW,
...
assumed to be identical to the far detector (maybe apart from the size) in order to minimize the impact of systematical uncertainties. The near detector is simulated implicitly by lower effective systematical errors,
and there are no external backgrounds assumed.
...
individual uncertainties (see \Ref~\cite{Huber:2003pm}). The energy resolution is
...
\subsection*{Double Chooz -- {\tt D-Chooz\_near.glb} and {\tt D-Chooz\_far.glb}}
\index{aedl}{D-Chooznear.glb@{\tt D-Chooz\_near.glb}}
\index{aedl}{D-Choozfar.glb@{\tt D-Chooz\_far.glb}}

The files {\tt D-Chooz\_near.glb} and {\tt D-Chooz\_far.glb} allow to simulate the \DC\ reactor experiment in France. They require user-defined systematics and \GLOBES\ 3.0 or higher, where the user-defined
systematics function can be found in the header of  {\tt D-Chooz\_near.glb} or {\tt example5.c}.
...
\begin{tabular}{lllllll} 
\hline 
Experiment & File & Runtime & $\gamma$ & Baseline & Det. & Mass \\ \hline \hline
Low $\gamma$ & {\tt BB\_100.glb} & 4~yr $\nu$/4~yr $\bar{\nu}$ & 100 & 130~km & WC & 500~kt \\ 
Medium $\gamma$ & {\tt BB\_350.glb} & 4~yr $\nu$/4~yr $\bar{\nu}$ & 350 & 730~km & WC & 500kt \\
Variable $\gamma$ & {\tt BBvar\_WC.glb} & 4~yr $\nu$/4~yr $\bar{\nu}$ & variable & variable & WC & 500~kt \\  
Variable $\gamma$ & {\tt BBvar\_TASD.glb} & 4~yr $\nu$/4~yr $\bar{\nu}$ & variable & variable & TASD & 50~kt \\ 
\hline 
\end{tabular}
\end{center}
...
four migration matrices (for $\nu_e , \bar{\nu}_e ,\nu_{\mu} ,\bar{\nu}_\mu$) that describe the energy smearing 
due to Fermi Motion. The following rules are defined within {\tt BB\_100.glb}:
\begin{center}
\begin{tabular}{|l|ll|c|c|}
\hline \hline
...
dependent efficiencies and background rejection factors. They are taken from the appendix of
...
Technically {\tt EXP\_FACTOR} can be chosen completely free, but the value should not deviate far from zero to stay
meaningful. See \cite{Huber:2005jk} for a detailed discussion of this parameter. The $\gamma$ value has to be chosen above 50. {\tt BBvar\_WC.glb} is optimized for $\gamma\lesssim350$ but
should be satisfactory up to $\gamma=500$. 
 The baseline is
L~=~{\tt baselinefactor}$\cdot\gamma$~km, the
fiducial mass of the detector is $\mathrm{m_{det} = 500 \,kt}$ and 4~years $\nu$-running and 4~years
$\bar{\nu}$-running are assumed. The {\sf AEDL}-Variable {\tt baselinefactor} must be chosen such that the baseline lies
in the interval $\mathrm{1\,km\lesssim\, L \, \lesssim\, 2\cdot R_{EARTH}}$. The appearance measurement involves the total rates data from all CC events and the spectral data
...
\begin{tabular}{|l|ll|c|c|}
\hline \hline
\multicolumn{3}{|l|}{Appearance -- $^{18}$Ne stored -- Spectrum} & $\sigma_\mathrm{norm}$ & $\sigma_\mathrm{cal}$\\ \hline
Signal & $0.55 \, \otimes \, (\nu_e \rightarrow \nu_\mu)_\mathrm{QE}$ & \hspace{6cm} & 10.0 & $10^{-4}$ \\
 & & & & \\
...
$(100/\gamma)^\alpha\cdot2.2\cdot10^{18}$ $^{18}$Ne decays per
year and $(60/\gamma)^\alpha\cdot5.8\cdot10^{18}$ $^{6}$He decays per year are assumed where $\alpha$~=~{\tt
EXP\_FACTOR} is a parameter that describes ion decay scaling. As default value {\tt EXP\_FACTOR=0} should be chosen. Technically {\tt EXP\_FACTOR} can be chosen completely free, but the value should not deviate far from zero to stay
meaningful. See \cite{Huber:2005jk} for a detailed discussion of this parameter. The $\gamma$ value has to be 
chosen above 80. The baseline is L~=~{\tt baselinefactor}$\cdot\gamma$~km, the
fiducial mass of the detector is $\mathrm{m_{det} = 50 \,kt}$ and 4~years $\nu$-running and 4~years
$\bar{\nu}$-running are assumed. 
The {\sf AEDL}-Variable {\tt baselinefactor} must be chosen such that the baseline lies
in the interval $\mathrm{1\,km\lesssim\, L \, \lesssim\, 2\cdot R_{EARTH}}$.
The energy resolution is $\mathrm{\sigma_e=6 \% \cdot \sqrt{E}}$ for electrons and  $\mathrm{\sigma_e=3 \% \cdot \sqrt{E}}$
...
\begin{tabular}{lllllll}
\hline 
Experiment & File & Runtime & $\mathrm{E_\mu}$ & Baseline & Det. & Mass \\ \hline \hline
Standard & {\tt NFstandard.glb} & \begin{tabular}{l}4~yr $\nu$ \\ 4~yr $\bar{\nu}$\end{tabular} & 50~GeV & 3000~km & \begin{tabular}{l}MID\end{tabular} & \begin{tabular}{l}50~kt\end{tabular} \\ 
Variable $E_\mu$ & {\tt NFvar.glb} & \begin{tabular}{l}4~yr $\nu$ \\ 4~yr $\bar{\nu}$\end{tabular} & variable & variable & \begin{tabular}{l}MID\end{tabular} & \begin{tabular}{l}50~kt\end{tabular} \\ 
Gold+Silver & {\tt NF\_GoldSilver.glb} & \begin{tabular}{l}4~yr $\nu$ \\ 4~yr $\bar{\nu}$\end{tabular} & variable & variable
&\begin{tabular}{l}MID\\ECC\end{tabular}&\begin{tabular}{l}50~kt\\5~kt\end{tabular}\\
Hybrid det. & {\tt NF\_hR\_lT.glb} & \begin{tabular}{l}4~yr $\nu$ \\ 4~yr $\bar{\nu}$\end{tabular} & variable & variable & \begin{tabular}{l}Hybrid\end{tabular} & \begin{tabular}{l}50~kt\end{tabular} \\ 
\hline
\end{tabular}
\end{center}
}
...
used within \cite{Huber:2002mx} (NuFact-II scenario, but some changes in normalization errors). 
This reference should be cited if the file {\tt
...
$\mathrm{E_\mu = 50\,GeV}$. A number of $1.06\cdot10^{21}$ useful muon decays per year is assumed in each
...
\subsection*{Variable $\boldsymbol{E_\mu}$ Neutrino Factory -- {\tt NFvar.glb}}
...
\ei
The neutrino beam is produced by the decay of muons stored in a storage ring at a parent energy of
$\mathrm{E_\mu =}$~{\tt emax}. The parent energy of the muons can be appropriately set in the range
$\mathrm{10\,GeV\lesssim E_\mu\lesssim 80\, GeV}$. The baseline of the scenario is L~=~{\tt BASELINE} and has to be chosen within the interval $\mathrm{1\,km\lesssim\, L \, \lesssim\, 2\cdot R_{EARTH}}$. 
Besides these settings the other attributes of {\tt NFvar.glb} are similar to the ones from {\tt
NFstandard.glb}. Only the treatment of the disappearance channels is different. In the disappearance channels of {\tt NFstandard.glb}, the charge identification is used to reduce the background from the appearance neutrinos. In {\tt NFvar.glb}, however, a threshold similar to the MINOS experiment~\cite{Ables:1995wq} is applied and the appearance and disappearance rates are assumed to be indistinguishable for the disappearance channel. The following rules are defined within
{\tt NFvar.glb}:
\begin{center}
\begin{tabular}{|l|ll|c|c|}
\hline \hline
...
\subsection*{Variable Feutrino Factory with Silver Channel -- \\{\tt NF\_GoldSilver.glb}}
...
and the values of the following {\sf AEDL}-Variables have to be set:
\bi
\item {\tt emax} (parent energy of the stored muons $\left[\mathrm{GeV}\right]$)
\item {\tt BASELINE} (experiment baseline $\left[\mathrm{km}\right]$)
\ei
The beam and golden channel attributes are similar to {\tt NFvar.glb}. The parent energy of the muons can be appropriately
set in the range $\mathrm{10\,GeV\lesssim E_\mu\lesssim 80\, GeV}$ (technically it is possible to arrange {\tt emax} for $\mathrm{E_\mu=5\,
GeV}$). The baseline is set by the {\sf AEDL}-Variable {\tt BASELINE} and has to be chosen within the interval $\mathrm{1\,km\lesssim\, L \, \lesssim\, 2\cdot R_{EARTH}}$. For the silver channel, an additional ECC detector with a fiducial mass
...
required and the {\sf AEDL}-Variables {\tt emax} and {\tt BASELINE} have to be set within the same constraints as for {\tt
NFvar.glb}. {\tt NF\_hR\_lT.glb} 
...
\multicolumn{3}{|l|}{Disappearance -- $\mu^+$-stored} & $\sigma_\mathrm{norm}$ & $\sigma_\mathrm{cal}$ \\ \hline
Signal & $0.9 \, \otimes \, (\bar{\nu}_\mu \rightarrow \bar{\nu}_\mu)_\mathrm{CC}$ & $0.9 \, \otimes \,
(\nu_e \rightarrow \nu_\mu)_\mathrm{CC}$ \hspace{1.6cm} & 0.025 & $10^{-4}$ \\
 & & & & \\
Background & $1.0 \, \otimes \, (\bar{\nu}_\mu \rightarrow \bar{\nu}_x)_\mathrm{NC}$ & & 0.2 & $10^{-4}$ \\ \hline \hline
\end{tabular}
\end{center}
...
unit could be {\it megawatts on target (MW)}, again this number is directly
