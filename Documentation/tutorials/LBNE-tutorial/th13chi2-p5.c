 ...
 *                                                                         *
 * Solution to problem 5: Implementation of the tracking algorithm         *
 ***************************************************************************/
...
int main(int argc, char *argv[])
{
  ...
  glb_params min_loc      = glbAllocParams();
  ...
  for(x=x_lower; x <= x_upper+0.00001; x+=delta_x)
  {
    ...
    chi2=glbChiTheta13(test_values, min_loc, GLB_ALL);
    fprintf(outfile, " %15.10g %15.10g\n", pow(10.0,x), chi2);
  }
 
  fclose(outfile);
  glbFreeParams(min_loc);
