...
interpreted as zero central value. IS THAT CORRECT [WW]?}
...
\index{aedl}{{\tt rule}!sys on function@{\tt "@sys\_on\_function}}
\index{aedl}{{\tt rule}!sys off function@{\tt "@sys\_off\_function}}
\index{norm}{Systematics function}
...
\index{aedl}{{\tt rule}!sys on errors@{\tt "@sys\_on\_errors}}
\index{aedl}{{\tt rule}!sys off errors@{\tt "@sys\_off\_errors}}
\begin{quote}
{\tt
...
\end{quote}
WHICH SYSTEMATICAL ERRORS WILL A FUNCTION USE IF THERE IS A MIXED DECLARATION,
SUCH AS WITH SYS\_ON\_ERRORS IN SYS ON AND SIGNALERROR/BACKGROUNDERROR in SYS OFF?
DOES THAT WORK? [WW]
