...
The channels are the building blocks for the so-called ``rules''. In general, a rule consists of one or more ``signal'' and ``background'' oscillation channels, which are normalized with efficiencies
...
file names are always quoted in \AEDL.\index{File names}
...
\index{Oscillation!switching off}
It is also possible to define a channel as no-oscillation by using the
prefix \GLB{NOSC\_} in either the initial  flavour or 
...
marked as \GLB{NOSC\_} are only computed by a call to \GLB{glbSetRates}
and not recomputed in the subsequent fit (which calls \GLB{glbSetNewRates}).
...
be specified with \GLB{\$sampling\_stepsize}
\begin{quote}
{\tt
...
with each rule to an energy window\index{Energy window}:
\begin{quote}
{\tt 
...
error dimension variable\index{Error dimension}, and the different 
...
by the value of \GLB{@errordim\_sys\_off}, whereas the error dimension
for systematics on is given by \GLB{@errordim\_sys\_on}. Thus, for example,
