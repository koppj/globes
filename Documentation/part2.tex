...
\index{aedl}{bincenter@{\tt bincenter}} and {\tt simbincenter} \index{aedl}{simbincenter@{\tt simbincenter}} return lists with the central energies of the bins or sampling points, respectively. For example,
\begin{quote}
{\tt
 \%bc=bincenter() 
}
...
\index{aedl}{{\tt nuflux}!time@{\tt "@time}}
...
\index{aedl}{{\tt nuflux}!builtin@{\tt "@builtin}}
\index{aedl}{{\tt nuflux}!parent energy@{\tt "@parent\_energy}}
\index{aedl}{{\tt nuflux}!stored muons@{\tt "@stored\_muons}}
\index{aedl}{{\tt nuflux}!end point@{\tt "@end\_point}}
\index{aedl}{{\tt nuflux}!stored ions@{\tt "@stored\_ions}}
\index{aedl}{{\tt nuflux}!gamma@{\tt "@gamma}}
\begin{quote}
{\tt nuflux(\#mu\_plus)<\\
\tb  @builtin = 1\\
\tb  @parent\_energy = 50.0\\
...
\end{quote}
%
\index{aedl}{{\tt nuflux}!power@{\tt "@power}}
\index{aedl}{{\tt nuflux}!norm@{\tt "@norm}}
\index{aedl}{{\tt nuflux}!flux file@{\tt "@flux\_file}}
...
The first option (method ``T'') is (JK: IS THAT CORRECT? AND ALSO THE NEXT EQUATION? (THERE HAVE BEEN
SOME CHANGES) [WW])
\begin{equation}
...
{\tt chiNoSysSpectrum} &-&-&-&-&-&2&No systematics\\
...
{\tt chiSpectrumCalib} &+&+&+&+&C&9&ystematics with calibr.\\
...
\end{quote}
If none of the built-in algorithms listed in \Tab~\ref{tab:error_dim} is
suitable for a certain setup, the user can override them by setting
{\tt @errordim\_sys\_on} to 20 and using the \GLB{SetUserChi}
function to specify a specialized function for the calculation
of $\chi^2$. This function will then be called by the \GLOBES\ minimizers.
For details, see \Sec~\ref{sec:userchi}.
