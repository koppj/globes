...
file names are always quoted in \AEDL.\index{norm}{File names}
...
marked as {\tt NOSC\_} are only computed by a call to \GLB{SetRates}
and not recomputed in the subsequent fit (which calls \GLB{SetNewRates}).
...
by the value of {\tt @errordim\_sys\_off}\index{aedl}{{\tt rule}!{\tt "@errordim\_sys\_off}}, whereas the error dimension
for systematics on is given by {\tt @errordim\_sys\_on}
\index{aedl}{{\tt rule}!{\tt "@errordim\_sys\_on}}. Thus, for example,
...
called {\tt globes}\index{norm}{{\tt globes}}, which can be seen either as a 
crude \AEDL\ debugger 
...
\section{Basic usage of {\tt globes}}
\label{sec:globes_basics}

The {\tt globes}\index{norm}{{\tt globes}} binary is installed along 
with the library \index{norm}{{\tt libglobes}}, but
...
\index{norm}{{\tt globes}!oscillation parameters}
Calling {\tt globes} without any options and with a {\tt .glb}-file as argument produces an event summary at rule level, taking into account 
...
{\tt GLB\_CENTRAL\_VALUES}\index{norm}{{\tt  GLB\_CENTRAL\_VALUES}}.
\begin{quote}
{\tt
...
\index{norm}{{\tt globes}!warnings}\index{norm}{{\tt globes}!errors}
\index{norm}{{\tt globes}!verbosity}
...
\index{norm}{{\tt globes}!total rates}
\index{norm}{{\tt globes}!spectral rates}
...
\index{norm}{{\tt globes}!channel rates}
...
\index{norm}{{\tt globes}!rule rates}
...
\index{norm}{{\tt globes}!variable substitution}
