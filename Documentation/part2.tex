...
Thus, a simple definition of a channel\index{aedl}{channel@{\tt channel}} is
\begin{quote}
...
\begin{equation}
s_i(a,b) \equiv s_i(a)+b\cdot s_i \cdot i / N_\mathrm{bins}.
\end{equation}
It is often refered to as a ``tilt'' of the
...
It is based upon replacing the events in the $i$th bin (mean reconstructed energy $E'_i$)
...
\end{eqnarray}
Here, $E'_\mathrm{min} = {\tt \$emin}$,  $\Delta E_0$ is the bin width
({\tt \$emax}-{\tt \$emin})/{\tt \$bins},
