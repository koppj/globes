...
\item[Experiment combination] An experiment combination consists of different experiments with different detectors, baselines, and (uncorreleted) matter density profiles. The result of an experiment combination
...
the user to break up the strict coupling of systematics with rules. For example, systematics can be
...
%
Further changes in \AEDL\ are the requirement of a minimum \GLOBES\ version the \AEDL\ is running with,
and the extension of lists as \AEDL\ variables and an interpolation feature.
...
changed or updated from eariler versions, such as for T2K and T2HK, or the standard neutrino
...
systematics, and the beta beams built-in beta beam fluxes. Both of these features are only
...
New platform and interpreter support, new ways to improve the performance
