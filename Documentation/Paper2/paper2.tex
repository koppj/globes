...
\section{Simulation of non-standard physics}

The probability calculation in \GLOBES\ 
...
For example, for the signal and background normalization errors, the rates in
each bin $i$ are multiplied with
\begin{equation}
...
This function can be defined with user-defined systenatics. Similarly, calibration errors, shape
errors, uncorrelated bin-to-bin errors, \etc , can be introduced. Furthermore, more complicated setups using multiple discrete sources and detectors can be simulated. For details, we refer to the manual~\cite{Manual}.
