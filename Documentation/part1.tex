...
requires some knowledge about the experiment definition. \GLOBES\
currently supports rule-based and channel-based event rate functions,
where the information is in most cases written into a file.
The rule-based functions are:
\begin{function}
...
\end{function}
\begin{function}
\GLBNS{glbShowTotalRuleRates}
{\tt double glbShowTotalRuleRates(int exp, int rule, int pos,
int effi, int bgi, int coeffi, int signal)} returns the total rates with
the same parameters as {\tt glbShowRuleRates}.
\end{function}
(THIS SYNTAX NEEDS TO BE CHECKED, BECAUSE FUNCTION IS NOT YET IMPLEMENTED - WW)

The function {\tt glbShowTotalRuleRates} is especially useful if
...
In addition, \GLOBES\ has a channel-based rate function:
\begin{function}
...
\begin{function}
\GLBNS{glbXSection}
{\tt double glbXSection(int ident, double E, int l, int anti)} returns
the cross section of cross section number {\tt ident} for the flavor $\nu_l$ 
and polarity {\tt anti} ($+1$: neutrinos, $-1$: antineutinos) at the energy {\tt E}.
\end{function}
Note that all flux or flavor numbers run from $0$ to $N-1$ in these functions.

(THIS FUNCTION MAY GET AN EXPERIMENT INDEX IF NOT THROUGH-COUNTED IN AEDL!)
