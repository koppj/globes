...
parameter vectors $(\theta_{12}, \theta_{13}, \theta_{23}, \deltacp, \sdm, \ldm)$, the parameter type \GLB{\_params} is used. In general,
 this type is often
...
To specify a user-defined $\chi^2$ algorithm, the following function can be used:
\begin{function}
\GLBNS{SetUserChi}
...
to calculate $\chi^2$ for experiment {\tt exp} and rule {\tt rule} (these parameters
can take the value \GLBC{GLB\_ALL} to specify all experiments or all rules). {\tt dim} is
...
Deviating from the standard conventions, the entries of {\tt x[]} will start at
index 1.
\end{function}
...
Note that {\tt chi\_func} will be called many times by the \GLOBES\ minimizers.
...
\end{function}
\begin{function}
\GLBNS{ShiftBackgroundEnergyScale}
...
Typical applications for user-defined systematics are reactor neutrino experiments,
...
\example{Systematics for a reactor setup with near and far detector}{
The following code fragment calculates $\chi^2$ for a reactor setup with
a far detector (experiment 0) and a near detector (experiment 1),
including six different systematical errors: the flux normalization
of the reactor ({\tt x[1]}), the fiducial mass uncertainties of the far
({\tt x[2]}) and near ({\tt x[3]}) detector, the normalization of the
background ({\tt x[4]}) and energy calibration errors for the far
({\tt x[5]}) and near ({\tt x[6]}) detectors. We assume that the central
values and errors of these parameters are stored in the global arrays
{\tt sys\_centers} and {\tt sys\_errors}. The calculation follows
\equ{chi2reactor}.

\begin{quote}{\tt \footnotesize
  double chi2nearfar(double *x)                                                 \\
  \{                                                                            \\
\hspace*{0.5 cm}      double chi2 = 0;                                          \\
\hspace*{0.5 cm}      double chirate, measured\_rate;                           \\
\hspace*{0.5 cm}                                                                \\
\hspace*{0.5 cm}      glbShiftSignalEnergyScale(0, 0, x[5]);   /* Apply E calib. error */ \\
\hspace*{0.5 cm}      glbShiftSignalEnergyScale(1, 0, x[6]);                    \\
\hspace*{0.5 cm}      glbShiftBackgroundEnergyScale(0, 0, x[5]);                \\
\hspace*{0.5 cm}      glbShiftBackgroundEnergyScale(1, 0, x[6]);                \\
...
\hspace*{0.5 cm}                                                                \\
\hspace*{0.5 cm}      /* The function is called for each exp ->
                                       Assign chi\textasciicircum2/2 to each */ \\
\hspace*{0.5 cm}      return chi2 / 2;                                          \\
  \}                                                                            
}
\end{quote}
}
...
\index{norm}{External information!starting values}
...
parameters. In \equ{priors}, $\hat{\rho}^0$ is the {\em starting value} of the 
