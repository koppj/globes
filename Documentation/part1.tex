...
experiment number ($0$ to \GLB{\_num\_of\_exps}-1) for a specific experiment. The format of \GLB{\_params} is discussed in detail in \Chapt~\ref{chapt:gettingstarted}. Note that all functions but {\tt ChiSys}
  are using minimizers which have to be initialized with \GLB{SetInputErrors} and \GLB{SetStartingValues} first.}
\end{table}
...
supporting files, are first of all searched in the current directory, and then in the global path variable \GLBC{GLB\_PATH} set in your environment.
...
on (\GLBC{GLB\_ON}) or off (\GLBC{GLB\_OFF}).
\end{function}
\begin{function}
