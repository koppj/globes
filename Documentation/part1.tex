...
experiment number ($0$ to \GLB{\_num\_of\_exps}-1) for a specific experiment. The format of \GLB{\_params} is discussed in detail in \Chapt~\ref{chapt:gettingstarted}. Note that all functions but {\tt glbChiSys} are using minimizers which have to be initialized with \GLB{SetInputErrors} and \GLB{SetStartingValues} first.}
\end{table}
...
An example of a minimal \GLOBES\ program in C can be found on page~\pageref{ex:c}. Furthermore, the files of the examples in this part can be found in the {\tt Example} subdirectory of your \GLOBES\ distribution. \index{norm}{Examples} Since it depends on your installed
...
\index{norm}{{\tt glb}-files} 
Pre-defined experiment prototypes, their filenames (to be used in \GLB{InitExperiment}), their short descriptions, and the references in which they are originally used and discussed (except from minor modifications, such as a
...
supporting files, are first of all searched in the current directory, and then in the global path variable 
%
...
\index{norm}{Clear experiment list}
\index{norm}{Delete experiments}
...
parameter vectors $(\theta_{12}, \theta_{13}, \theta_{23}, \deltacp, \sdm, \ldm)$, the parameter type \GLB{\_params\_type} is used. In general, pointers to this structure are of the type \GLB{\_params}, and are often
...
\end{function} 
\begin{function}
\GLBNS{SetIteration} \index{norm}{Iterations}
...
that it cannot be altered and must not be destroyed by the
user.
\end{function}
...
\index{norm}{Two-parameter correlation}
...
\index{norm}{Correlations $\chi^2$}

\index{norm}{Multi-parameter correlation}
...
In practice, this projection is very difficult: a grid-based method would need $(N_{\mathrm{grid}})^n$ function calls of {\tt glbChiSys} to calculate the precision including the full $n$-parameter correlation, where $N_{\mathrm{grid}}$ is the number of points in each direction of the lattice. For example, taking only $N_{\mathrm{grid}}=20$ and $n=7$ (six oscillation parameters and matter density) would mean more than one billion function calls of {\tt glbChiSys}. One can easily imagine that these
would be too many for any reasonable application.
...
\index{norm}{Priors} \index{norm}{Input errors} \index{norm}{Starting values}
...
to converge into the unwanted best-fit solution. However, note that one should in this case check that the actually determined value for $\ldm$ after minimization is close enough to the guessed value $-\ldm$ in order to avoid significant artifical contributions of the priors to the 
final $\chi^2$.

\index{norm}{Input errors} \index{norm}{Starting values}
In order to set the starting values and input errors, two function have to
 be called {\em before the usage of any minimizer}:
\begin{function}
\GLBNS{SetStartingValues}
...
\item
 ...
 using \GLB{ChiNP}. A more subtile consequence is the comparison
...
\index{norm}{Projection onto axis}
...
% 
Note that before any minimization, \GLB{SetStartingValues} 
and \GLB{SetInputErrors} have to be used at least once. In addition, note that the resulting $\chi^2$ of {\tt glbChiTheta} (or {\tt glbChiDelta}) for the combination of more than one experiment is not 
...
are present, since each experiment may face other matter density conditions. The minimizers of individual experiments ``know'' which
...
number, the last density value will be modified.
...
Other examples for projections onto a parameter axis while keeping exactly one parameter fixed are \GLB{ChiTheta23}, \GLB{ChiDm}, and \GLB{ChiDms}, which
...
\index{norm}{Projection onto hyperplane}
...
minimized over. 
The corresponding function is \index{norm}{Projection onto $\theta_{13}$-$\deltacp$-plane}
\begin{function}
\GLBNS{ChiThetaDelta}
...
\hline
\GLB{AllocProjection} & Create projection vector & {\tt ()} \\
\GLB{FreeProjection} & Destroy projection vector {\tt stale} & {\tt (glb\_projection stale)} \\
...
\index{norm}{Degenerate solutions}
\index{norm}{Degeneracies $\chi^2$}
...
In many cases, one can find the exact position of the $\mathrm{sgn}(\ldm)$-degeneracy with \GLB{ChiAll}, where one starts 
...
\index{norm}{All-parameter minimization}
...
In this case, the position of the degeneracy can be easily guessed to be at the best-fit parameter values but the inverted $\ldm$. The minimizer then runs off the plane of these parameters into the local minimum. It is very important to take into account the position of the degeneracy off this plane, since the actual $\chi^2$ in the minimum is probably lower than on the plane of the best-fit parameter values. Thus, the degeneracy may not even appear at the chosen confidence level on the plane, but it does appear at the true minimum. The two sections through the fit manifold shown in the figure on page~\pageref{ex:sgndeg} therefore do not appear at the same oscillation parameter values (except from the ones shown in the 
...
\index{norm}{Matter density profile: Change}
