...
\GLBNS{glbSetDensityProjectionFlag} {\tt glbSetDensity-} \newline {\tt ProjectionFlag}  & Set flag for density parameter {\tt which} in vector {\tt in} to value {\tt flag}. & {\tt (glb\_projection in, int flag, int which)} \\
\GLBNS{glbGetDensityProjectionFlag} {\tt glbGetDensity-} \newline {\tt ProjectionFlag} & Return flag for density parameter {\tt which} in vector {\tt in}.  & {\tt (const glb\_projection in, int which)} $\rightarrow$ {\tt int} flag \\
\hline
\end{tabular}
\caption{\label{tab:defprojection} Different functions handling the
\GLB{glb\_projection} type. Flags are either \GLB{GLB\_FIXED} or \GLB{GLB\_FREE}.}
\end{table}

In principle, one can also use three- or more-dimensional projections. In addition, one may want to use any other set of parameters for single- or two-parameter projections. The function \GLB{glbChiNP} is designed for this purpose.

WW: CONTINUE HERE

glb\_projection type: exactly similar to glb\_params (return values)

Functions in table: komische Aufteilung
