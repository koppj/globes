...
%
The file name of an \AEDL\ file can be retrived with\footnote{The following two functions are available in \GLOBES\ 3.1.7 and higher.}
\begin{function}
\GLBNS{GetFilenameOfExperiment}
{\tt const char *glbGetFilenameOfExperiment(int experiment)} returns the file name of the \AEDL\ file for the experiment number {\tt experiment}. Note that the returned pointer cannot be modified.
\end{function}
and the references to be cited with
\begin{function}
\GLBNS{GetCitationForExperiment} \label{glbGetCitationForExperiment}
{\tt const char *glbGetCitationForExperiment(int experiment)} returns the citation defined in the \AEDL\ file for the experiment number {\tt experiment}. Note that the returned pointer cannot be modified. See also {\tt \$citation} in \AEDL\ on  page~\pageref{citation}.
\end{function}
...
\end{function}
\begin{function}
\GLBNS{GetEnergyWindow} \label{glbGetEnergyWindow}
...
\end{function}
\begin{function}
\GLBNS{GetCoefficientInRule} \label{glbGetCoefficientInRule}
...
of the experiment {\tt exp}.
The parameter {\tt signal} refers to signal (\GLBC{GLB\_SIG}) or background
(\GLBC{GLB\_BG}). See also {\tt glbSetCoefficientInRule} on page~\pageref{glbSetCoefficientInRule}.
\end{function}
%
...
\begin{function}
\GLBNS{SetCoefficientInRule} \label{glbSetCoefficientInRule}
...
The parameter {\tt signal} refers to signal (\GLBC{GLB\_SIG}) or background
(\GLBC{GLB\_BG}). See also {\tt glbGetCoefficientInRule} on page~\pageref{glbGetCoefficientInRule}.
\end{function}
...
\begin{function}
\GLBNS{SetEnergyWindow} \label{glbSetEnergyWindow}
...
A related function which allows to name an oscillation engine without invoking it
\begin{function}
  \GLBNS{DefineOscEngine}
{\tt int glbDefineOscillationEngine(int n\_parameters,
                 glb\_probability\_matrix\_function prob\_func, \\
                 glb\_set\_oscillation\_parameters\_function set\_params\_func, \\
                 glb\_get\_oscillation\_parameters\_function get\_params\_func, \\
                 const char[] name,\\
                 void* user\_data)} defines a probability
engine for the simulation of non-standard physics with {\tt n\_parameters} oscillation parameters ({\tt n\_parameters} $\ge6$). The three functions {\tt prob\_func}, {\tt set\_params\_func}, and {\tt get\_params\_func} 
will be used with the function types defined above. In order to circumvent global variables, an arbitrary pointer {\tt user\_data} can be defined, that will be passed to the probability engine in each function call. {\tt name} can be use to refer to the so defined oscillation engine within {\sf AEDL} using the {\tt \$oscillation\_engine} directive.
\end{function}
...
\section{Defining oscillation parameter names at running times}

One can either define constants for the non-standard physics parameters, as in our example
\begin{quote}
{\tt
  const int GLB\_SIGMA\_E = 6;  
}
\end{quote}
or assign and read out names dynamically. This feature is available in \GLOBES\ 3.1.7 and higher. The following functions are supported:
\begin{function}
\GLBNS{SetParamName}
{\tt int glbSetParamName(const char *name, int i)} assigns {\tt name} to the oscillation parameter {\tt i} (starting at 0 for {\tt GLB\_THETA\_12}).  
\end{function}
As usual, the return value is $0$ ({\tt GLB\_SUCCESS}) if successful.
Note that the string is copied, which means that the original pointer can be modied or destroyed thereafter.
\begin{function}
\GLBNS{SetParamNames}
{\tt int glbSetParamNames(char **names)} assigns names to all of the oscillation parameters at once. The array of strings {\tt names} must be of length {\tt glbGetNumOfOscParams()}. Individual entries are allowed to be {\tt NULL}.
\end{function}
\begin{function}
\GLBNS{GetParamName}
{\tt char *glbGetParamName(int i)} returns the name of the oscillation parameter {\tt i}.  
\end{function}
\begin{function}
\GLBNS{FindParamByName}
{\tt int glbFindParamByName(const char *name)} returns the number of the oscillation parameter named {\tt name}.
\end{function}
\begin{function}
\GLBNS{SetOscParamByName}
{\tt int glbSetOscParamByName(glb\_params in, double value, const char *name)} sets an oscillation parameter in the structure {\tt in} to {\tt value}. Compared to the similar function {\tt glbSetOscParams}, which parameter is to be accessed is determined by the {\tt name}, not the number.
\end{function}
\begin{function}
\GLBNS{GetOscParamByName}
{\tt double glbGetOscParamByName(glb\_params in, const char *name)} returns an oscillation parameter in the structure {\tt in}. Compared to the similar function {\tt glbGetOscParams}, which parameter is to be accessed is determined by the {\tt name}, not the number.
\end{function}
\begin{function}
\GLBNS{ClearParamNames}
{\tt int glbClearParamNames()} clears all names assigned to oscillation parameters and returns the allocated memory.
\end{function}
